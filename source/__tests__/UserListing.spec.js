...
const addUsers = async (activeUserCount, inactiveUserCount = 6) => {
...
describe('Listing Users', () => {
  ...
  it('returns 6 users in page content when there are active 6 users and inactive 5 users in databse', async () => {
    await addUsers(6, 5);
    const response = await getUsers();
  ...
  it('returns 5 users and corresponding size indicator when size is set as 5 un request parameter', async () => {
    await addUsers(11);
...
describe('Get User', () => {
  ...
  it('returns 200 ok when as active user exist', async () => {
  ...
  it('returns id, username ans email when as active user exist', async () => {
    const user = await User.create({
      username: 'user1',
      email: 'user1@mail.com',
      inactive: false,
