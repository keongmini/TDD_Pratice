...
const activeUser = { username: 'user1', email: 'user1@email.com', password: 'Password123', inactive: false };
...
describe('Authentication', () => {
  it('returns 200 when credentials are correct', async () => {
    await addUser();
    const response = await postAuthentication({ email: 'user1@email.com', password: 'Password123' });
  ...
  `('returns $message when authentication fails and language is set as $language', async ({ language, message }) => {
    const response = await postAuthentication({ email: 'user1@email.com', password: 'Password123' }, { language });
    expect(response.body.message).toBe(message);
  });

  it('returns 401 when password is wrong', async () => {
    await addUser();
    const response = await postAuthentication({ email: 'user1@email.com', password: 'password' });
    expect(response.status).toBe(401);
  });
  ...
  it('returns 401 when email is not valid', async () => {
    const response = await postAuthentication({ password: 'Password123' });
    expect(response.status).toBe(401);
  });

  it('returns 401 when password is not valid', async () => {
    const response = await postAuthentication({ email: 'user1@email.com' });
    expect(response.status).toBe(401);
  });
});
